...
import com.yuuki1293.bookbook.common.entity.ItemBookEntity
...
class BookItem(properties: Properties) extends Item(properties) {
  ...
  override def createEntity(level: Level, location: Entity, stack: ItemStack): Entity = {
    val itemBook = new ItemBookEntity(EntityType.ITEM, level, location.getX, location.getY, location.getZ, stack)
